...
function roleFromBackend(session: AuthSessionDto): Role[] {
  ...
  if (hasAnyRole(roleIds, ['Job Creation', 'JC', 'Job Controller', 'CW Job Creation'])) roles.add('Job Creation')
  ...
  if (hasAnyRole(roleIds, ['Parts', 'CW Parts', 'Store Keeper', 'Picker', 'Estimator', 'Parts Buyer / Estimator', 'Parts Manager'])) roles.add('Parts')
