import { Add, DeleteOutlined, Edit, Print, Refresh, Search, Send } from '@mui/icons-material'
...
export function ServiceOrdersPage() {
  ...
  const [exception, setException] = useState<{ row: ServiceOrder; kind: 'amend' | 'cancel' } | null>(null)
  ...
  function printDocument(doctype: 'Sales Order' | 'Sales Invoice', name?: string) {
    if (!name) { toast.warning(`No ${doctype} is linked to this Service Order yet.`); return }
    window.open(workshopApi.workshopDocumentPrintUrl(doctype, name), '_blank', 'noopener,noreferrer')
  }
  function openException(row: ServiceOrder, kind: 'amend' | 'cancel') { setException({ row, kind }); setExceptionReason('') }
  async function confirmException() {
    if (!exception || !exceptionReason.trim()) { toast.warning('Record a reason for this controlled action.'); return }
    setSaving(true)
    try {
      if (exception.kind === 'amend') await workshopApi.amendServiceOrder(exception.row.id, exceptionReason.trim())
      else await workshopApi.cancelServiceOrder(exception.row.id, exceptionReason.trim())
      toast.success(exception.kind === 'amend' ? 'A new Service Order version was created.' : 'Service Order cancelled.')
      setException(null); await load()
    } catch (cause) { toast.error(cause, exception.kind === 'amend' ? 'Unable to amend Service Order.' : 'Unable to cancel Service Order.') }
    finally { setSaving(false) }
  }
  ...
  return <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1480, mx: 'auto', minWidth: 0 }}><Stack spacing={3}>
    ...
    <Stack spacing={1.5}>{visible.map((row) => { const customer = customers.find((item) => item.id === row.customerId); const vehicle = vehicles.find((item) => item.id === row.vehicleId); return <Box key={row.id} sx={{ p: { xs: 1.75, sm: 2.25 }, border: `1px solid ${colors.border.default}`, borderRadius: radii.lg, bgcolor: colors.bg.card, boxShadow: shadows.card, minWidth: 0 }}><Stack direction={{ xs: 'column', lg: 'row' }} spacing={2} sx={{ justifyContent: 'space-between', alignItems: { lg: 'center' } }}><Box sx={{ minWidth: 0 }}><Stack direction="row" spacing={1} sx={{ alignItems: 'center', flexWrap: 'wrap' }}><Typography sx={{ fontWeight: 850 }}>{row.id}</Typography><Chip size="small" label={row.status}/></Stack><Typography sx={{ color: colors.slate[600] }}>{vehicle?.registrationNo || row.vehicleId} · {customer?.fullName || row.customerId}</Typography><Typography sx={{ color: colors.slate[500], fontSize: '.78rem', overflowWrap: 'anywhere' }}>Appointment {row.appointmentId} · {row.lines.length} line(s) · {row.jobIds?.length || 0} Job Card(s) · {row.currency} {row.grandTotal.toLocaleString()}</Typography></Box><Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1 }}>{['Draft','Estimate Prepared','Revision Required'].includes(row.status) ? <Button size="small" variant="outlined" startIcon={<Edit/>} onClick={() => beginEdit(row)}>Edit</Button> : null}{['Draft','Estimate Prepared','Revision Required'].includes(row.status) ? <Button size="small" variant="contained" startIcon={<Send/>} onClick={() => void submit(row)}>Submit</Button> : null}{row.status === "Customer Approval Pending" ? <><Button size="small" variant="contained" color="success" onClick={() => void decide(row, "Approved")}>Customer approved</Button><Button size="small" color="error" onClick={() => void decide(row, "Rejected")}>Customer rejected</Button></> : null}{["Approved","In Execution","Partially Completed","Completed"].includes(row.status) ? <Button size="small" variant="outlined" onClick={() => void generateJobs(row)}>Sync Job Cards</Button> : null}{["Customer Approval Pending","Approved","In Execution","Partially Completed","Completed"].includes(row.status) && !row.salesInvoiceId ? <Button size="small" variant="outlined" onClick={() => openException(row, "amend")}>Amend</Button> : null}{!["Cancelled","Amended","Invoiced","Closed"].includes(row.status) && !row.salesInvoiceId ? <Button size="small" color="error" onClick={() => openException(row, "cancel")}>Cancel</Button> : null}{["Partially Completed","Completed"].includes(row.status) && !row.salesInvoiceId ? <Button size="small" variant="outlined" onClick={() => void requestClearance(row, "Invoice Review")}>Invoice review</Button> : null}<Button size="small" variant="outlined" startIcon={<Print/>} disabled={!row.salesOrderId} title={row.salesOrderId ? `Print ${row.salesOrderId}` : 'Sales Order is created after approval'} onClick={() => printDocument('Sales Order', row.salesOrderId)}>Sales Order</Button><Button size="small" variant="outlined" startIcon={<Print/>} disabled={!row.salesInvoiceId} title={row.salesInvoiceId ? `Print ${row.salesInvoiceId}` : 'Sales Invoice is created after completion'} onClick={() => printDocument('Sales Invoice', row.salesInvoiceId)}>Invoice</Button>{row.salesInvoiceId ? <Button size="small" variant="outlined" onClick={() => void requestClearance(row, "Payment Confirmation")}>Payment confirmation</Button> : null}{["Invoiced","Completed"].includes(row.status) ? <Button size="small" variant="outlined" color="success" onClick={() => void requestClearance(row, "Final Release")}>Release clearance</Button> : null}</Stack></Stack></Box>})}{!loading && !visible.length ? <Box sx={{ textAlign: 'center', py: 8 }}><Typography sx={{ fontWeight: 800 }}>No Service Orders found</Typography><Typography sx={{ color: colors.slate[500] }}>Create one from an appointment to establish the controlled commercial scope.</Typography></Box> : null}</Stack>
  </Stack>
  <Dialog open={Boolean(exception)} onClose={() => !saving && setException(null)} fullWidth maxWidth="sm"><DialogTitle sx={{ fontWeight: 850 }}>{exception?.kind === 'amend' ? 'Create Service Order amendment' : 'Cancel Service Order'}</DialogTitle><DialogContent dividers><Stack spacing={2}><Alert severity={exception?.kind === 'amend' ? 'info' : 'warning'}>{exception?.kind === 'amend' ? 'The approved version and Job Card history stay immutable. A new draft version will be created.' : 'Cancellation is allowed only before execution or financial posting starts.'}</Alert><TextField autoFocus required multiline minRows={4} label="Reason" value={exceptionReason} onChange={(event) => setExceptionReason(event.target.value)} placeholder="Explain the customer or operational reason."/></Stack></DialogContent><DialogActions><Button onClick={() => setException(null)} disabled={saving}>Back</Button><Button variant="contained" color={exception?.kind === 'cancel' ? 'error' : 'primary'} onClick={() => void confirmException()} disabled={saving || !exceptionReason.trim()}>{saving ? 'Saving…' : exception?.kind === 'amend' ? 'Create amendment' : 'Cancel order'}</Button></DialogActions></Dialog>
